...
import { GetSrcFilesFunction, Runtime, RUNTIME, ZipFunction, CpioFunction } from '../runtime.js'
...
import { cpioNodeJs } from './utils/cpio.js'
...
const cpioFunction: CpioFunction = async function ({
  ...
  // If the file is a cpio, we assume the function is bundled and ready to go.
  // We simply copy it to the destination path with no further processing.
  if (extension === '.cpio') {
    const destPath = join(destFolder, filename)
  ...
  const generator = mergedConfig?.generator || getInternalValue(isInternal)
  const cpioResult = await cpioNodeJs({
    aliases,
    archiveFormat,
    basePath: finalBasePath,
    branch,
  ...
  return {
    bootstrapVersion: cpioResult.bootstrapVersion,
    bundler: bundlerName,
    bundlerWarnings,
    config: mergedConfig,
    displayName: mergedConfig?.name,
    entryFilename: cpioResult.entryFilename,
    generator,
    timeout: mergedConfig?.timeout,
    inputs,
    includedFiles,
...
const cpioWithFunctionWithFallback: CpioFunction = async ({ config = {}, ...parameters }) => {
  // If a specific JS bundler version is specified, we'll use it.
  if (config.nodeBundler !== NODE_BUNDLER.ESBUILD_ZISI) {
    return cpioFunction({ ...parameters, config })
  }

  // Otherwise, we'll try to bundle with esbuild and, if that fails, fallback
  // to zisi.
  try {
    return await cpioFunction({ ...parameters, config: { ...config, nodeBundler: NODE_BUNDLER.ESBUILD } })
  } catch (esbuildError) {
    try {
      const data = await cpioFunction({ ...parameters, config: { ...config, nodeBundler: NODE_BUNDLER.ZISI } })
...
const runtime: Runtime = {
  ...
  cpioFunction: cpioWithFunctionWithFallback,
}
