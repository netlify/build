...
import { ARCHIVE_FORMAT, NODE_BUNDLER, cpioFunction } from '../src/main.js'
...
describe('cpioFunction', () => {
  describe('v1', () => {
    testMany(
      'Resolves dependencies from .netlify/plugins/node_modules when using `cpioFunction()`',
      [...allBundleConfigs],
      async (options) => {
        const { path: tmpDir } = await getTmpDir({ prefix: 'zip-it-test' })
        const mainFile = join(FIXTURES_DIR, 'node-module-next-image', 'function', 'function.js')
        const result = (await cpioFunction(mainFile, tmpDir, options))!

        expect(result).not.toBeUndefined()

        const unzippedFunctions = await extractCpioFiles([result])
    ...
    testMany(
      'Includes includedFiles in the response of cpioFunction',
      [...allBundleConfigs, 'bundler_none'],
      async (options) => {
        const { path: tmpDir } = await getTmpDir({ prefix: 'zip-it-test' })
        const mainFile = join(FIXTURES_DIR, 'node-module-next-image', 'function', 'function.js')
        const result = (await cpioFunction(mainFile, tmpDir, {
          ...options,
    ...
    testMany('Can use cpioFunction()', [...allBundleConfigs, 'bundler_none'], async (options, variation) => {
    ...
    test('When generating a directory for a function with `archiveFormat: "none"`, it empties the directory before copying any files', async () => {
      ...
      await cpioFunction(`${FIXTURES_DIR}/simple/function.js`, tmpDir, {
    ...
    test('Creates dynamic import shims for functions using `cpioFunction`', async () => {
      const { path: tmpDir } = await getTmpDir({ prefix: 'zip-it-test' })
      const fixtureDir = join(FIXTURES_DIR, 'node-module-dynamic-import-2')
      const result = (await cpioFunction(join(fixtureDir, 'function.js'), tmpDir, {
        basePath: fixtureDir,
    ...
    testMany(
      ...
      async (options) => {
        ...
        const result = (await cpioFunction(`${basePath}/function-1.js`, tmpDir, opts))!
  ...
  describe('v2', () => {
    ...
    test('Does not log to systemlog for v1', async () => {
      ...
      await cpioFunction(mainFile, tmpDir, {
        systemLog,
      })
