    ...
    parseDecorator() {
      ...
      for (; ; ) {
        if (this.eat(tt.dot)) {
          ...
          node2.object = expr;
          node2.property = this.type === tt.privateId ? this.parsePrivateIdent() : this.parseIdent(true);
          node2.computed = false;
          expr = this.finishNode(node2, "MemberExpression");
        } else if (
          // TypeScript lets a non-null assertion punctuate the chain: `@x!`, `@x!.y`.
          !this.hasPrecedingLineBreak() && this.value === "!" && this.match(tt.prefix)
        ) {
          this.exprAllowed = false;
          this.next();
          const node2 = this.startNodeAt(startPos, startLoc);
          node2.expression = expr;
          expr = this.finishNode(node2, "TSNonNullExpression");
        } else {
          break;
        }
      }
...
function isPossiblyLiteralEnum(expression) {
  ...
  if (computed && !(property.type === "Literal" && typeof property.value === "string") && (property.type !== "TemplateLiteral" || property.expressions.length > 0)) {
    return false;
  }
...
function tsPlugin(options) {
  ...
  return function(Parser) {
    ...
    class TypeScriptParser extends EffectParser {
       ...
       */
      exprAllowedAfterDeclarationBody() {
        this.exprAllowed = true;
      }
      ...
      tsParseImportEqualsDeclaration(node, isExport) {
        ...
        this.checkLValSimple(
          node.id,
          node.importKind === "type" || !isExternalModule ? acornScope.BIND_FLAGS_TS_EXPORT_ONLY : acornScope.BIND_LEXICAL
        );
      ...
      parseFunction(node, statement, allowExpressionBody, isAsync, forInit) {
        ...
        if (statement & FUNC_STATEMENT) {
          node.id = statement & FUNC_NULLABLE_ID && !tokenIsIdentifier(this.type) ? null : this.parseIdent();
        }
      ...
      parseImport(node) {
        ...
        if (tokenIsIdentifier(enterHead.type) || this.match(tt.star) || this.match(tt.braceL)) {
          ...
          if (aheadIsFrom) {
            ...
            aheadIsBindingNamedFrom = this.isContextualWithState("from", afterAhead) || afterAhead.type === tt.eq;
          }
          if (
            // import type, { a } from "b";
            ahead.type !== tt.comma && // import type from "a";
            (!aheadIsFrom || aheadIsBindingNamedFrom) && // import type = require("a");
        ...
        this.next();
        if (node.importKind === "value" && this.type === tt.name && this.value === "defer" && !this.containsEsc && this.lookahead().type === tt.star) {
          node.phase = "defer";
          this.next();
        }
        if (this.type === tt.string) {
      ...
      reportReservedArrowTypeParam(node) {
        if (node.params.length === 1 && !node.extra?.trailingComma && !node.params[0]?.constraint && disallowAmbiguousJSXLike) {
      ...
      parseImportSpecifier() {
        ...
        if (this.importOrExportOuterKind === "type") {
          ...
          this.checkLValSimple(node2.local, acornScope.BIND_TS_TYPE);
      ...
      parseTypeOnlyImportExportSpecifier(node, isImport, isInTypeOnlyImportExport) {
        ...
        if (isImport) {
          this.checkLValSimple(
            node[rightOfAsKey],
            node[kindKey] === "type" || isInTypeOnlyImportExport ? acornScope.BIND_TS_TYPE : acornScope.BIND_FLAGS_TS_IMPORT
          );
        }
      }
      raiseCommonCheck(pos, message, recoverable) {
        if (this.isAmbientContext && (/^The keyword '.*' is reserved$/.test(message) || /^Binding \w+ in strict mode$/.test(message) || /^Assigning to \w+ in strict mode$/.test(message) || /^Cannot use keyword 'await' outside an async function$/.test(message) || /^Cannot use 'await' as identifier inside an async function$/.test(message) || /^Cannot use \w+ in class static initialization block$/.test(message) || /^Cannot use 'arguments' in class field initializer$/.test(message) || message === "Classes can't have a static field named 'prototype'")) {
          return;
        }
        switch (message) {
          case "'import' and 'export' may appear only with 'sourceType: module'": {
            ...
            if (this.type === tt._import) {
              ...
              if (ahead.type === tokTypes.type && tokenIsKeywordOrIdentifier(ahead2.type) && this.lookahead(3).type === tt.eq) {
                return;
              }
            }
            break;
