...
const slugifyPackageName = (specifier: string) => {
  if (!specifier.startsWith('@')) return specifier.replace(/\//g, '_')
...
const safelyDetectTypes = async (pkg: string, basePath: string): Promise<string | undefined> => {
  const lookupPath = basePath.endsWith(path.sep) ? basePath : basePath + path.sep
  try {
    const aa = packageName(pkg)
    const json = await findUp(`node_modules/${aa}/package.json`, {
      cwd: lookupPath,
    })
    if (json) {
    ...
    for (const i of imports) {
      ...
      switch (i.moduleSpecifier.type) {
        ...
        case 'package': {
          ...
          } else if (!matched && !resolvedImport?.protocol?.startsWith('http')) {
