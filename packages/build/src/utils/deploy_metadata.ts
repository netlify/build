...
export const DeployMetadataSchema = z.object({
  environment: z
    .array(z.object({ key: z.string().min(1), value: z.string(), isSecret: z.boolean().default(false) }))
    .default(() => []),
})

const DEPLOY_METADATA_FILENAME = 'deploy-metadata.json'

export const getDeployMetadataPath = (buildDir: string) => resolve(buildDir, '.netlify', DEPLOY_METADATA_FILENAME)
