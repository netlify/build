...
type SiteInfo = { id?: string | undefined; account_id?: string | undefined }

// `@netlify/config` adds `functionsDirectory`, which the public `NetlifyConfig` type leaves out
type ResolvedNetlifyConfig = NetlifyConfig & { functionsDirectory?: string | undefined }
...
export const getConstants = async function ({
  ...
  netlifyConfig: ResolvedNetlifyConfig
  siteInfo: SiteInfo
...
type ConstantsInput = { [Name in keyof NetlifyPluginConstants]?: NetlifyPluginConstants[Name] | undefined }
...
export const addMutableConstants = async function ({
  ...
  // `getConstants()` sets the other required constants, except `SITE_ID`, which is `undefined` without a site
  return constantsC as NetlifyPluginConstants
}
...
// configured it.
const addDefaultConstants = async function (constants: ConstantsInput, buildDir: string) {
  ...
  return newConstants.reduce<ConstantsInput>(
    (constantsA, newConstant) => Object.assign(constantsA, newConstant),
    Object.assign({}, constants),
  )
}

type DefaultPathConstantName = 'FUNCTIONS_SRC' | 'EDGE_FUNCTIONS_SRC' | 'DB_MIGRATIONS_SRC'

// The current directory is the build directory, which is correct, so we don't
// need to resolve paths
const DEFAULT_PATHS: { constantName: DefaultPathConstantName; defaultPath: string }[] = [
...
const addDefaultConstant = async function ({
  constants,
  constantName,
  defaultPath,
  buildDir,
  ...
  constants: ConstantsInput
  constantName: DefaultPathConstantName
  defaultPath: string
  buildDir: string
}): Promise<ConstantsInput> {
...
const normalizeConstantsPaths = function (constants: ConstantsInput, buildDir: string) {
  const normalizedConstants = { ...constants }
  CONSTANT_PATHS.filter((name) => Object.hasOwn(normalizedConstants, name)).forEach((name) => {
    normalizedConstants[name] = normalizePath(normalizedConstants[name], buildDir)
  })
...
const normalizePath = function (path: string | undefined, buildDir: string) {
