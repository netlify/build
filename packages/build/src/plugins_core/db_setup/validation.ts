...
export const formatValidationErrors = (errors: ValidationError[]): string => {
  const lines = errors.map((error) => {
    if (error.type === 'invalid_dir_name') {
      return `  - "${error.dirName}" does not match the required pattern "<number>_<slug>" (e.g. "1700000000_create-users", "001_create-users", or "001_create_users"). Slugs must be lowercase alphanumeric with hyphens or underscores.`
    }
