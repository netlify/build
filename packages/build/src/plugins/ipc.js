...
const getEventFromChild = async function(childProcess, expectedEvent) {
  if (!childProcess.connected) {
    throw new Error(`Could not receive event '${eventName}' to child process because it already exited`)
  }
  ...
  if (eventName === 'error') {
    throw new Error(`Error in child process: ${payload.stack}`)
  }
...
const validateEventName = function(eventName, expectedEvent) {
  if (expectedEvent !== undefined && expectedEvent !== eventName) {
    throw new Error(`Expected event '${expectedEvent}'`)
  }
}
