...
export const logFunctionsToBundle = function ({
  ...
  if (generatedFunctions.length !== 0) {
    log(logs, `Packaging ${type} generated by extensions:`)

    const entries = generatedFunctions.map((func) => {
      const name = `${func.displayName || path.basename(func.path)}`

      return func.generator ? `${name} (${func.generator})` : name
    })
    logArray(logs, entries, { indent: false })
  }
