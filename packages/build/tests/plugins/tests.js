...
test('Pass packageJson to plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/package_json_valid').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Pass empty packageJson to plugins if no package.json', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/package_json_none')
...
test('Pass empty packageJson to plugins if package.json invalid', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/package_json_invalid').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Can use pure ES modules with local plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/es_modules_local').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Can use pure ES modules with module plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/es_modules_module').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Can use CommonJS with local plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/commonjs_local').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Local plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/local').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Local plugins directory', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/local_dir').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Local plugins absolute path', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/local_absolute').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Local plugins invalid path', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/local_invalid').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Node module plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/module').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('UI plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/ui')
    .withFlags({
...
test('Resolution is relative to the build directory', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/module_base')
    .withFlags({
...
test('Resolution respects monorepo node module resolution rules', async (t) => {
  const fixture = new Fixture(test.meta.file, './fixtures/monorepo')
...
test('Non-existing plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/non_existing').runWithBuild()
  t.snapshot(normalizeOutput(output))
})
...
test('Validate --node-path unsupported version does not fail when no plugins are used', async (t) => {
  ...
  const output = await new Fixture(test.meta.file, './fixtures/empty').withFlags({ nodePath }).runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Validate --node-path version is supported by the plugin', async (t) => {
  const nodePath = getNodePath('16.14.0')
  const output = await new Fixture(test.meta.file, './fixtures/engines')
    .withFlags({
      nodePath,
      debug: false,
    })
...
test('Validate --node-path exists', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/node_version_simple')
...
test('Provided --node-path version is unused in buildbot for local plugin executions if older than supported version', async (t) => {
  ...
  const output = await new Fixture(test.meta.file, './fixtures/version_greater_than_minimum')
...
test('UI plugins dont use provided --node-path', async (t) => {
  const nodePath = getNodePath('12.19.0')
  const output = await new Fixture(test.meta.file, './fixtures/ui_auto_install')
    .withFlags({
      nodePath,
      mode: 'buildbot',
      defaultConfig: { plugins: [{ package: 'netlify-plugin-test' }] },
...
test('Plugin output can interleave stdout and stderr', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/interleave').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

// TODO: check output length once big outputs are actually fixed
test.serial('Big plugin output is not truncated', async (t) => {
  await new Fixture(test.meta.file, './fixtures/big').runWithBuild()
  t.pass()
})

test('Plugins can have inputs', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/inputs').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Trusted plugins are passed featureflags and system log', async (t) => {
  const systemLogFile = await tmpName()
  const output = await new Fixture(test.meta.file, './fixtures/feature_flags')
    .withFlags({
  ...
  const outputUntrusted = await new Fixture(test.meta.file, './fixtures/feature_flags_untrusted')
    .withFlags({
      featureFlags: { test_flag: true },
      debug: false,
      systemLogFile: await fs.open(systemLogFile, 'a'),
...
test('process.env changes are propagated to other plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/env_changes_plugin').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('process.env changes are propagated to onError and onEnd', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/env_changes_on_error').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('process.env changes are propagated to build.command', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/env_changes_command').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('build.environment changes are propagated to other plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/env_changes_build_plugin').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('build.environment changes are propagated to onError and onEnd', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/env_changes_build_on_error').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('build.environment changes are propagated to build.command', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/env_changes_build_command').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('build.environment and process.env changes can be mixed', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/env_changes_build_mix').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Expose some utils', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/keys').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Utils are defined', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/defined').runWithBuild()
  t.snapshot(normalizeOutput(output))
})
...
test('Can run utils', async (t) => {
  ...
  try {
    const output = await new Fixture(test.meta.file, './fixtures/functions_add').runWithBuild()
    t.snapshot(normalizeOutput(output))
  } finally {
    await removeDir(functionsDir)
  }
...
test('Cache utils are caching .dot directories as well', async (t) => {
  ...
  // generate cache
  await new Fixture(test.meta.file, './fixtures/cache_utils').runWithBuild()
  // should have cached files in the output message
  const output = await new Fixture(test.meta.file, './fixtures/cache_utils').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Can run list util', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/functions_list').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Git utils fails if no root', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/git_no_root')
    .withCopyRoot({ git: false })
...
test('Git utils does not fail if no root and not used', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/keys')
    .withCopyRoot({ git: false })
    .then((fixtures) => fixtures.runWithBuild())
  t.snapshot(normalizeOutput(output))
})
...
test('Validate plugin is an object', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/object').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Validate plugin event handler names', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/handler_name').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Validate plugin event handler function', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/handler_function').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Transpile TypeScript local plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/ts_transpile').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Type-checks TypeScript local plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/ts_type_check').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Type-checks TypeScript local plugins using tsconfig.json', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/ts_type_check_tsconfig').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Does not transpile already transpiled local plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/ts_transpile_already').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Plugins which export a factory function receive the inputs and a metadata object', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/dynamic_plugin').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Plugin events that do not emit to stderr/stdout are hidden from the logs', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/mixed_events').withFlags({ debug: false }).runWithBuild()
  t.snapshot(normalizeOutput(output))
})
...
test('Plugin errors that occur during the loading phase are piped to system logs', async (t) => {
  ...
  const output = await new Fixture(test.meta.file, './fixtures/syntax_error')
    .withFlags({
      debug: false,
...
test.serial('Plugins have a pre-populated Blobs context', async (t) => {
  ...
  const { netlifyConfig } = await new Fixture(test.meta.file, './fixtures/blobs_read')
    .withFlags({
      apiHost: host,
      deployId,
      testOpts: { scheme },
...
test('Plugins can respond anything to parent process', async (t) => {
  const build = await new Fixture(test.meta.file, './fixtures/process_send_object').runBuildBinary()
