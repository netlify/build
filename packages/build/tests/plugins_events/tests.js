...
test.only('Keeps output to a minimum in the `startDev` entrypoint when `quiet: true`', async (t) => {
  const devCommand = sinon.stub().resolves()
...
test.only('Shows error information in the `startDev` entrypoint even when `quiet: true`', async (t) => {
  const devCommand = sinon.stub().resolves()
