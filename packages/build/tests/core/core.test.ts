...
test('Passes the right properties to zip-it-and-ship-it', async () => {
  const mockZipFunctions = vi.spyOn(zipItAndShipIt, 'bundle').mockResolvedValue({ functions: [] })
  ...
  const params1 = mockZipFunctions.mock.calls[0][0]

  expect(params1.basePath).toBe(fixtureDir)
  expect(params1.functions?.config?.['*'].zipGo).toBe(true)
  expect(params1.functions?.config?.['*'].includedFilesBasePath).toBe(fixtureDir)
  expect(params1.repositoryRoot).toBe(fixtureDir)

  expect(params1.functions?.config?.['*'].nodeVersion).toBe(process.versions.node)

  const params2 = mockZipFunctions.mock.calls[1][0]

  expect(params2.functions?.config?.['*'].nodeVersion).toBe('nodejs00.x')
  expect(params2.functions?.config?.['*'].zipGo).toBe(undefined)
})
...
test('Passes functions generated by build plugins to zip-it-and-ship-it', async () => {
  ...
  const paths = mockZipFunctions.mock.calls[0][0].functions?.paths
...
test('Passes the right feature flags to zip-it-and-ship-it', async () => {
  ...
  expect(options1.functions?.config?.test.schedule).toBe('@daily')
...
test('`rustTargetDirectory` is passed to zip-it-and-ship-it only when running in buildbot', async () => {
  ...
  expect(call1Args[0].functions?.config?.['*'].rustTargetDirectory).toBe(
    join(FIXTURES_DIR, 'functions_config_1', '.netlify', 'rust-functions-cache', '[name]'),
  )
  expect(call2Args[0].functions?.config?.['*'].rustTargetDirectory).toBe(undefined)
  expect(call3Args[0].functions?.config?.['*'].rustTargetDirectory).toBe(
    join(FIXTURES_DIR, 'functions_internal_missing', '.netlify', 'rust-functions-cache', '[name]'),
  )
  expect(call4Args[0].functions?.config?.['*'].rustTargetDirectory).toBe(undefined)
})
...
test('configFileDirectories is passed to zip-it-and-ship-it', async () => {
  ...
  expect(call1Args[0].functions?.configFileDirectories).toEqual([
...
test('functions can have a config with different parameters passed to zip-it-and-ship-it', async () => {
  const zipItAndShipItSpy = vi.spyOn(zipItAndShipIt, 'bundle')
  ...
  const manifestPath = call1Args[0].manifest!
...
test('internalSrcFolder is passed to zip-it-and-ship-it and helps prefill the generator field', async () => {
  ...
  const [options] = call1Args
  const paths = options.functions?.paths
