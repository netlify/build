...
for (const { testPrefix, featureFlags } of [
  {
    testPrefix: 'scanning using node:readline (default) > ',
    featureFlags: {},
  },
  {
    testPrefix: 'scanning without buffering whole lines (feature flag) > ',
    featureFlags: {
      secret_scanning_no_readline: true,
    },
  },
]) {
  test(testPrefix + "secrets scanning, don't run when no secrets are provided/default", async (t) => {
    const output = await new Fixture('./fixtures/src_default').withFlags({ debug: false, featureFlags }).runWithBuild()
    t.false(normalizeOutput(output).includes('Scanning for secrets in code and build output'))
  })

  test(
    testPrefix + "secrets scanning, don't run when there are no secrets and enhanced scan not enabled",
    async (t) => {
      const { requests } = await new Fixture('./fixtures/src_default')
        .withFlags({ debug: false, explicitSecretKeys: '', deployId: 'test', token: 'test', featureFlags })
  ...
  test(
    testPrefix + "secrets scanning, don't run when no explicit secrets, enhanced scan enabled but no likely secrets",
    async (t) => {
  ...
  test(
    testPrefix +
      'secrets scanning, run and report result to API when there are no secrets and enhanced scan is enabled with likely secrets',
    async (t) => {
  ...
  test(testPrefix + 'secrets scanning, should skip with secrets but SECRETS_SCAN_ENABLED=false', async (t) => {
    const output = await new Fixture('./fixtures/src_scanning_disabled')
      .withFlags({ debug: false, explicitSecretKeys: 'ENV_VAR_1,ENV_VAR_2', featureFlags })
      .runWithBuild()
    t.true(normalizeOutput(output).includes('Secrets scanning disabled via SECRETS_SCAN_ENABLED flag set to false.'))
  })

  test(testPrefix + 'secrets scanning, should skip with enhanced scan but SECRETS_SCAN_ENABLED=false', async (t) => {
  ...
  test(testPrefix + 'secrets scanning, should skip when secrets passed but no env vars set', async (t) => {
  ...
  test(testPrefix + 'secrets scanning, should skip when enhanced scan enabled but no env vars set', async (t) => {
    const { requests } = await new Fixture('./fixtures/src_default')
      .withFlags({ debug: false, explicitSecretKeys: '', enhancedSecretScan: true, deployId: 'test', token: 'test' })
      .runBuildServer({ path: '/api/v1/deploys/test/validations_report' })
  ...
  test(
    testPrefix + 'secrets scanning, should skip when secrets passed but no non-empty/trivial env vars set',
    async (t) => {
  ...
  test(
    testPrefix + 'secrets scanning, should skip when enhanced scan enabled but no non-empty/trivial env vars set',
    async (t) => {
  ...
  test(
    testPrefix + 'secrets scanning, should skip when secrets passed but SECRETS_SCAN_OMIT_KEYS omits all of them',
    async (t) => {
  ...
  test(
    testPrefix +
      'secrets scanning, should skip when enhanced scan and likely secrets passed but SECRETS_SCAN_OMIT_KEYS omits all of them',
    async (t) => {
  ...
  test(
    testPrefix + 'secrets scanning, should skip when secrets passed but SECRETS_SCAN_OMIT_PATHS omits all files',
    async (t) => {
  ...
  test(
    testPrefix +
      'secrets scanning, should skip when enhanced scan and likely secrets passed but SECRETS_SCAN_OMIT_PATHS omits all files',
    async (t) => {
  ...
  test(
    testPrefix + 'secrets scanning, should skip when secrets passed but SECRETS_SCAN_OMIT_PATHS omits globbed files',
    async (t) => {
  ...
  test(
    testPrefix +
      'secrets scanning, should fail build and report to API when it finds secrets in the src and build output',
    async (t) => {
  ...
  test(
    testPrefix +
      'secrets scanning, should fail build and report to API when enhanced scan finds likely secret in the src and build output',
    async (t) => {
  ...
  test(testPrefix + 'secrets scanning should report success to API when no secrets are found', async (t) => {
  ...
  test(
    testPrefix + 'secrets scanning, should report success to API when enhanced scans finds no likely secrets',
    async (t) => {
      const { requests } = await new Fixture('./fixtures/src_scanning_env_vars_no_matches')
        .withFlags({
          debug: false,
  ...
  test(testPrefix + 'secrets scanning failure should produce an user error', async (t) => {
  ...
  test(testPrefix + 'secrets scanning, enhanced scanning failure should produce a user error', async (t) => {
  ...
  test(testPrefix + 'secrets scan does not send report to API for local builds', async (t) => {
    const { requests } = await new Fixture('./fixtures/src_scanning_env_vars_set_non_empty')
      .withFlags({
        debug: false,
        explicitSecretKeys:
  ...
  test(
    testPrefix + 'secrets scanning, should not fail if the secrets values are not detected in the build output',
    async (t) => {
  ...
  test(testPrefix + 'secrets scanning should not scan .cache/ directory', async (t) => {
  ...
  if (featureFlags.secret_scanning_no_readline) {
    test(testPrefix + 'does not crash if line in scanned file exceed available memory', async (t) => {
