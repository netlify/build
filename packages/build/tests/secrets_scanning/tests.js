...
for (const { testPrefix, featureFlags } of [
  ...
  },
  {
    ...
    featureFlags: {
      secret_scanning_minimal_chunks: true,
    },
  },
]) {
  test(testPrefix + 'secrets scanning, should not run when disabled', async (t) => {
    const output = await new Fixture('./fixtures/src_scanning_disabled')
      .withFlags({ debug: false, featureFlags })
      .runWithBuild()
  ...
  test(testPrefix + 'secrets scanning, should skip when secrets passed but no env vars set', async (t) => {
    const output = await new Fixture('./fixtures/src_default', featureFlags)
  ...
  test(testPrefix + 'secrets scanning, should run when enhanced scan enabled and no env vars set', async (t) => {
