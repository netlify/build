...
test('Clean stack traces of plugin event handlers', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/plugin').withFlags({ debug: false }).runWithBuild()
  t.is(getStackLinesCount(output), 1)
})

test('Do not clean stack traces in debug mode', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/plugin').withFlags({ debug: true }).runWithBuild()
  t.not(getStackLinesCount(output), 1)
})

test('Does not clean stack traces of exceptions', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/stack_exception')
    .withFlags({ debug: false })
    .runWithBuild()
  t.not(getStackLinesCount(output), 1)
})
...
test('Clean stack traces of config validation', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/config_validation')
    .withFlags({ debug: false })
    .runWithBuild()
  t.is(getStackLinesCount(output), 0)
})
...
test('Print stack trace of plugin errors', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/plugin_stack').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Print stack trace of plugin errors during load', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/plugin_load').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Print stack trace of build.command errors', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/command_stack').runWithBuild()
  t.snapshot(normalizeOutput(output))
})

test('Print stack trace of build.command errors with stack traces', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/command_full_stack').runWithBuild()
  t.snapshot(normalizeOutput(output))
})
