...
test('Context with context CLI flag', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/context_flag')
...
test('Context default value', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/context_default').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Context with branch CLI flag', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/branch')
...
test('Context with branch fallback', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/branch_fallback')
    .withFlags({ branch: '' })
...
test('Context deep merge', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/deep_merge').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Context array merge', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/array_merge').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Context merge priority', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/priority_merge')
    .withFlags({ branch: 'testBranch' })
    .runWithConfig()
  t.snapshot(normalizeOutput(output))
})
...
test('Using context does not reset plugins', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/context_reset').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Can use context properties for build.edge_functions', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/context_edge_functions_build').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Can use context properties for edge_functions', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/context_edge_functions_top').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Context with branch wildcard pattern feat/*', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/branch_wildcard')
