...
## functions.ignored_node_modules: does not contain paths outside of the root directory

> Snapshot 1

    `When resolving config file packages/config/tests/validate/fixtures/function_config_invalid_included_files_2/netlify.toml:␊
    Configuration property functions.*.included_files must only contain paths that are inside the root directory.␊
    ␊
    Invalid syntax␊
    ␊
      [functions]␊
...
## functions.ignored_node_modules: is array of strings
