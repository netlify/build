...
const isDefinedString = function (string: string | undefined) {
...
test('Sets LANG environment variable', async () => {
  const {
    env: { LANG },
  } = asConfig(await new Fixture(import.meta.url, './fixtures/empty').runWithConfigAsObject())
...
test('Sets BUILD_ID environment variable', async () => {
  const {
    ...
    await new Fixture(import.meta.url, './fixtures/empty').withFlags({ buildId: 'test-build' }).runWithConfigAsObject(),
  )
...
test('Sets config file environment variables', async () => {
  const {
    env: { TEST },
  } = asConfig(await new Fixture(import.meta.url, './fixtures/file_env').runWithConfigAsObject())
...
test('Sets config file empty environment variables', async () => {
  const {
    env: { TEST },
  } = asConfig(await new Fixture(import.meta.url, './fixtures/file_env_empty').runWithConfigAsObject())
  expect(TEST?.sources).toEqual(['configFile'])
...
test('Coerces environment variables to string', async () => {
  ...
  } = asConfig(await new Fixture(import.meta.url, './fixtures/file_env_not_string').runWithConfigAsObject())
...
test('Does not allow overridding readonly environment variables', async () => {
  const {
    env: { REVIEW_ID },
  } = asConfig(await new Fixture(import.meta.url, './fixtures/readonly').runWithConfigAsObject())
