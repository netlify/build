...
test('Configuration file - netlify.toml', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/toml').runWithConfig()
...
test('Configuration file - parsing error', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/parse_error').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Configuration file - valid backslash sequences in TOML should not fail', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/parse_backslash_valid').runWithConfig()
  t.snapshot(normalizeOutput(output))
...
test('Configuration file - invalid backslash sequences in double quote strings in TOML should fail', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/parse_backslash_double').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Configuration file - trailing backslashes in double quote strings in TOML should fail', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/parse_backslash_trailing_double').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Configuration file - invalid backslash sequences in multiline double quote strings in TOML should fail', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/parse_backslash_double_multiline').runWithConfig()
  t.snapshot(normalizeOutput(output))
})
...
test('Configuration file - invalid backslash sequences in single quote strings in TOML should not fail', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/parse_backslash_single').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Configuration file - invalid backslash sequences in multiline single quote strings in TOML should not fail', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/parse_backslash_single_multiline').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Configuration file - detecting invalid backslash sequences in TOML does not misinterpret equal signs', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/parse_backslash_equal').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Redirects - redirects file', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/redirects_file').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Redirects - redirects field', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/redirects_field').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Redirects - redirects file and redirects field', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/redirects_both').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Redirects - redirects file syntax error', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/redirects_file_error').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Redirects - redirects field syntax error', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/redirects_field_error').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Redirects - no publish field', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/redirects_no_publish').runWithConfig()
  t.snapshot(normalizeOutput(output))
})
...
test('Redirects - does not use redirects file when using inlineConfig with identical redirects', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/redirects_file')
...
test('Headers - file', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_file').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Headers - field', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_field').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Headers - file and field', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_both').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Headers - file syntax error', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_file_error').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Headers - field syntax error', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_field_error').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Headers - no publish field', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_no_publish').runWithConfig()
  t.snapshot(normalizeOutput(output))
})
...
test('Headers - does not use headers file when using inlineConfig with identical headers', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_file')
...
test('Headers - duplicate case in same path', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_duplicate_case_same').runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Headers - duplicate case in different paths', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/headers_duplicate_case_different').runWithConfig()
  t.snapshot(normalizeOutput(output))
})
