...
test('Prints some information in debug mode', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/simple').withFlags({ debug: true }).runWithConfig()
  t.snapshot(normalizeOutput(output))
})

test('Allow printing undefined in debug mode', async (t) => {
  const output = await new Fixture(test.meta.file, './fixtures/empty').withFlags({ debug: true }).runWithConfig()
  t.snapshot(normalizeOutput(output))
})
...
test('Prints colors', async (t) => {
  const {
    logs: { stderr },
  } = await new Fixture(test.meta.file, './fixtures/simple')
