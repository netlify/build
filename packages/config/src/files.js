...
const handleFiles = async function ({ config: { build, ...config }, repositoryRoot, baseRelDir }) {
  const buildA = resolveBuildPaths(build, REPOSITORY_RELATIVE_PROPS, repositoryRoot)
  const buildDir = await getBuildDir(repositoryRoot, buildA)
  const baseRel = baseRelDir ? buildDir : repositoryRoot
  const buildB = resolveBuildPaths(buildA, BUILD_DIR_RELATIVE_PROPS, baseRel)
  const configA = resolveFunctionsPath(config, baseRel)
...
const resolveBuildPaths = function (build, propNames, baseRel) {
...
const resolveFunctionsPath = function (config, baseRel) {
  if (!config.functions['*'] || !config.functions['*'].directory) {
    return config
  }
  ...
  return {
    ...
    functions: {
      ...
      '*': {
        ...config.functions['*'],
        directory: resolvePath(baseRel, config.functions['*'].directory),
      },
    },
  }
}
...
const DEFAULT_PATHS = [
  { object: (directory) => ({ functions: { '*': { directory } } }), defaultPath: 'netlify/functions' },
