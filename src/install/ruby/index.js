...
module.exports = async function installRuby(cwd, cacheDir, version) {
  ...
  try {
    ...
    rvm = await execa('rvm', ['use', druby, '>', '/dev/null', '2>&1'])
  } catch (err) {
  ...
  try {
    ...
    const rvmCreateCommand = await execa('rvm', ['--create', 'use', druby, '>', '/dev/null', '2>&1'])
  ...
  if (rvmCreate === 0) {
    ...
    try {
      // @TODO verify command
      await execa('rvm_install_on_use_flag=1', [
        'rvm', '--quiet-curl', '--create', 'use', druby
      ])
    } catch (err) {
      console.log(`Failed to install ruby version '${druby}'`)
      process.exit(1)
    }
  ...
  let gems
  try {
    const gemList = await execa('gem', ['list', '-i', '"^bundler$"', '>', '/dev/null', '2>&1'])
    gems = gemList.stdout
  } catch (err) {
